...
module "sns-topic-subscription" {
    source = "github.com/pbs/terraform-sns-topic-subscription-module?ref=x.y.z"
}
```
...
```bash
mkdir -p 'terraform/modules/sns-topic-subscription'
gh -R 'pbs/terraform-sns-topic-subscription-module' release download -p 'release.tar.gz' x.y.z
tar -xvf release.tar.gz -C 'terraform/modules/sns-topic-subscription'
rm -f release.tar.gz
```
...
```bash
git remote add -f sns-topic-subscription git@github.com:pbs/terraform-sns-topic-subscription-module.git
git subtree add --prefix terraform/modules/sns-topic-subscription sns-topic-subscription main --squash
```

If necessary, pin the repo version, rather than adding `main` as your subtree:

```bash
git subtree add --prefix terraform/modules/sns-topic-subscription sns-topic-subscription $REF --squash
```
